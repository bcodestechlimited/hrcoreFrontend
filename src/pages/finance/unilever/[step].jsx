...
const UnileverPreview = () => {
	...
	const Arr1 = [
		{
			title: "Invoice No",
			value: `${
				invoiceCode?.all?.docs?.find(item => item?.type === "unilever")?.code ||
				""
			}${state?.invoiceId || ""}`,
		},
		{
	...
	return (
		<div>
			<div className="bg-[#EFF6FC] printOnly w-full px-4 py-8">
				...
				<div className="mt-10 bg-white rounded-xl w-full p-8">
					<div className="flex justify-between">
						...
						<div className="flex gap-5">
							...
							<button
								onClick={() =>
									navigate("/finance/unilever/update", { state: state?._id })
								}
					...
					{/* <div className="bg-[#F9FAFB] p-3 mt-8 border border-[#EAECF0] rounded-xl h-20 w-full">
						<h5 className="text-sm font-normal text-[#19202C] manrope">
							Additional Quotes
						</h5>
				...
				<div className="mt-10 bg-white pb-10">
					<table className="w-full text-sm text-gray-500 dark:text-gray-400">
						<thead className="text-xs text-[#013468] text-center capitalize whitespace-nowrap bg-gray-50">
							<tr cl>
...
export default UnileverPreview;
const PrintableDocs = React.forwardRef(
	(
		...
		// 	},
		// ];
		// const maxDescriptions = Math.max(
		// 	...tableData.map(item => item.description.length)
		// );
		// const Quotes = [
		// 	"APP 966",
		// 	"APP 966",
			...
			<section
				...
				<div className="flex justify-end mt-3 pe-6">
					<div className="space-y-1">
						{Arr1?.map(item => (
							<h3 className="text-sm text-black manrope font-bold">
								{item?.title === "TIN"
									? item?.value
										? item?.title
										: ""
				...
				<div className="mt-3">
					...
					{state?.contacts?.[0]?.attention && (
						<h3 className="text-sm pt-2 text-black manrope font-medium">
							Attention:{" "}
					...
					<table className="w-full mt-3 border-collapse border border-black">
						<thead className="capitalize text-sm text-white text-center whitespace-nowrap bg-black">
							<tr>
								...
								<th className="border border-black border-x-white p-2">vrn</th>
						...
						<tbody>
							...
							<tr
								...
								<td className="border border-black p-2">{item?.vrn}</td>
								<td className="border border-black p-2">{item?.model}</td>
								<td className="">
									{item?.description?.map((it, x) => (
										<div key={x} className="grid grid-cols-2">
											<p className="border border-black p-2">{it}</p>
											<p className="border border-black p-2">{it}</p>
										</div>
									))}
								</td>
